...
def test(model, val_loader):
	...
	total = 0
	correct = 0
	samples = json.load(open('./AVQA/data/json/avqa-test.json', 'r'))
	A_ext=[]
	A_count = []
	A_cmp = []
	A_temp=[]
...
def main():
	...
	if args.mode == 'train':
		train_dataset = AVQA_dataset(label=args.label_train, audio_dir=args.audio_dir, video_dir=args.video_dir, video_res14x14_dir=args.video_res14x14_dir,
									transform=transforms.Compose([ToTensor()]), mode_flag='train')
		train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)
		val_dataset = AVQA_dataset(label=args.label_val, audio_dir=args.audio_dir, video_dir=args.video_dir, video_res14x14_dir=args.video_res14x14_dir,
		...
		####### concat model
		pretrained_file = "./MUSIC-AVQA_CVPR2022/grounding_gen/models_grounding_gen_test/main_grounding_gen_2_best.pt"
