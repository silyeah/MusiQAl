...
def test(model, test_loader, run_nr = 0, intv_mode = None, training_data = None):
...
def main():
    ...
    if isinstance(args.training_data, str):
        model.load_state_dict(torch.load(args.model_save_dir + args.training_data + '_' + args.checkpoint + ".pt"))
        test(model, test_loader, run_nr = 0, intv_mode = args.intv_mode, training_data = args.training_data)
