...
def train(args, model, train_loader, optimizer, criterion, epoch, progress_epoch_filename, intv_mode):
    model.train()
    total_qa = 0
    correct_qa = 0

    # progress_filename = f'progress/{intv_mode}_avst_progress.csv'
...
def main():
    ...
    if args.mode == 'train':
        ...
        progress_epoch_filename = f'progress/{args.intv_mode}_avst_epoch_progress_lr.csv'
        eval_filename = f'progress/{args.intv_mode}_avst_epoch_eval_lr.csv'
        ...
        for epoch in range(1, args.epochs + 1):
            train(args, model, train_loader, optimizer, criterion, epoch=epoch, progress_epoch_filename=progress_epoch_filename, intv_mode = args.intv_mode)

            scheduler.step(epoch)
            F = eval(model, val_loader, epoch, eval_filename)
            if F >= best_F:
                best_F = F
                torch.save(model.state_dict(), args.model_save_dir + args.intv_mode + '_' + args.checkpoint + ".pt")
                print('Saved model')
        ...
        model.load_state_dict(torch.load(args.model_save_dir + args.intv_mode + '_' + args.checkpoint + ".pt"))
